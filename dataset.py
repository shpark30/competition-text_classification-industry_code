...
def bootstrap(
    data, estimators, dist="same", num_samples=100000, min_cat_data=2, seed=42
):
    """
    ...
    def _sampling_same(data, seed, total_samples, min_cat_data):
        ...
        for lb in data["label"].unique().tolist():
            ...
            if n_sampled < 0:
                ...
                pdb.set_trace()
            sub_data = pd.concat(
                [sub_data, data_lb.sample(n=n_sampled, random_state=seed_lb)]
            )
    ...
    def _sampling_random(data, seed, total_samples, min_cat_data, cut_std=1000):
        ...
        i = 0
        while True:
            dist["n_samples"] = dist.apply(
                lambda x: rand_num(x[1], min_cat_data, seed=seed + x[0] + i), axis=1
            )
            ...
            if sum_samples > total_samples:
                ...
                if cut_samples <= total_samples:
                    ...
                    for lb, n in zip(
                        *[
                            to_cut_df_dist["label"].tolist(),
                            to_cut_df_dist["n_samples"].tolist(),
                        ]
                    ):
                        to_cut_df = pd.concat(
                            [to_cut_df, pd.DataFrame({"label": [lb] * n})]
                        )
                    to_cut_df = to_cut_df.reset_index(drop=True)
                    adj_cut_df = to_cut_df.sample(n=save_num, random_state=42)
                    adj_cut_df_dist = (
                        adj_cut_df["label"].value_counts().to_frame().reset_index()
                    )
                    adj_cut_df_dist.columns = ["label", "cnt"]
                    for lb, n in zip(
                        *[
                            adj_cut_df_dist["label"].tolist(),
                            adj_cut_df_dist["cnt"].tolist(),
                        ]
                    ):
                    ...
                    ), f"sum_samples should be same as total_samples, {sum_samples} != {total_samples}."
                    break
                # cut_samples > total_samples
                cut_std -= 100
                continue
        ...
        if cut_std < 1000:
            print(
                f"decrease cut_std since cut_samples > total_samples. {1000} -->> {cut_std}"
            )
        sub_data = pd.DataFrame()
        for lb in data["label"].unique().tolist():
    ...
    for estimator in tqdm(
        range(estimators), total=estimators
    ):  # estimators 만큼 서브데이터 생성
    ...
    for lb in labels:
        ...
        while True:
            ...
            if method == "random":
                ...
                not_empty_cond = (
                    sample[["text_obj", "text_mthd", "text_deal"]]
                    .applymap(lambda x: len(x) != 0)
                    .apply(any, axis=1)
                )
            ...
            if method == "shuffle":
                ...
                sample = pd.concat(
                    [
                        temp_df["text_obj"]
                        .sample(n=n_sample, replace=True, random_state=s1)
                        .reset_index(drop=True),
                        temp_df["text_mthd"]
                        .sample(n=n_sample, replace=True, random_state=s2)
                        .reset_index(drop=True),
                        temp_df["text_deal"]
                        .sample(n=n_sample, replace=True, random_state=s3)
                        .reset_index(drop=True),
                    ],
                    axis=1,
                )
...
def preprocess(
    frame, num_test=100000, upsample="", minimum=500, clean_fn=None, target="S", seed=42
):
    """
    input
    ...
    if clean_fn:  # text 전처리 함수
        frame[["text_obj", "text_mthd", "text_deal"]] = frame[
    ...
    # labeling
    frame["digit_2"] = frame["digit_2"].apply(
        lambda x: num2code(x, 2)
    )  # 중분류를 2자리 코드값으로 변환
    frame["digit_3"] = frame["digit_3"].apply(
        lambda x: num2code(x, 3)
    )  # 소분류를 3자리 코드값으로 변환
    ...
    # input : 'text' column
    # target: 'label' column
    train["text"] = train[["text_obj", "text_mthd", "text_deal"]].apply(
        lambda x: " ".join(x), axis=1
    )
    test["text"] = test[["text_obj", "text_mthd", "text_deal"]].apply(
        lambda x: " ".join(x), axis=1
    )
...
class EnsembleDataset(Dataset):
    def __init__(
        ...
        mlbert_tokenizer=None,
        kobart_tokenizer=None,
        asbart_tokenizer=None,
        kogpt2_tokenizer=None,
        kogpt3_tokenizer=None,
        electra_tokenizer=None,
        funnel_tokenizer=None,
        max_len=50,
        padding="max_length",
        truncation=True,
    ):
        ...
        self.kobert_tokenized = (
            []
            if self.kobert_tokenizer is None
            else [self.kobert_tokenizer([d]) for d in doc]
        )
        self.bert_tokenized = (
            []
            if self.bert_tokenizer is None
            else self.bert_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
        ...
        self.mlbert_tokenized = (
            []
            if self.mlbert_tokenizer is None
            else self.mlbert_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.albert_tokenized = (
            []
            if self.albert_tokenizer is None
            else self.albert_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.kobart_tokenized = (
            []
            if self.kobart_tokenizer is None
            else self.kobart_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.asbart_tokenized = (
            []
            if self.asbart_tokenizer is None
            else self.asbart_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.kogpt2_tokenized = (
            []
            if self.kogpt2_tokenizer is None
            else self.kogpt2_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.kogpt3_tokenized = (
            []
            if self.kogpt3_tokenizer is None
            else self.kogpt3_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.electra_tokenized = (
            []
            if self.electra_tokenizer is None
            else self.electra_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
        ...
        self.funnel_tokenized = (
            []
            if self.funnel_tokenizer is None
            else self.funnel_tokenizer(
                doc,
                padding=padding,
                max_length=max_len,
                truncation=truncation,
...
class KobertClassifyDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (
            {
                "input_ids": input_ids,  # numpy.array
