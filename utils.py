...
def get_optimizer(
    optimizer_type, model, lr, betas, weight_decay, eps=1e-08, amsgrad=False
):
    ...
    # Not Matched
    raise Exception(f"{optimizer_type} is not available")


def create_logger(
    log_path, name="torch", file_name="train.log", fmt="%(asctime)s | %(message)s"
):
...
class Evaluator:
    ...
    def update(self, pred, label, loss=None):
        ...
        batch_confusion_matrix = confusion_matrix(
            label, pred, labels=list(range(self.num_classes))
        )
    ...
    def compute(self):
        ...
        self.acc = (
            np.diagonal(self.confusion_matrix).sum() / self.confusion_matrix.sum()
        )
        for c in range(self.num_classes):
            ...
            F1 = (
                2 * PC * RC / (PC + RC) if (PC + RC) != 0 else 0
            )  # (2 * PC * RC) / (PC + RC)
...
def save_performance_graph(summary_path, save_path):
    ...
    best_acc_epoch = data.loc[
        data["valid loss"].tolist().index(data["valid loss"].min()), "epoch"
    ]
    best_loss_epoch = data.loc[
        data["valid acc"].tolist().index(data["valid acc"].max()), "epoch"
    ]
...
if __name__ == "__main__":
    ...
    #                     help='path to data')
    parser.add_argument(
        "--num_test", default=100000, type=int, help="the number of test data"
    )
    parser.add_argument(
        "-j",
        "--workers",
