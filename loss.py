...
def get_loss(loss_type, **kwargs):
    ...
    if loss_type == "FCE":
        return FocalCrossEntropy(**kwargs)

    if loss_type == "ICE":
        return InverseFrequencyCrossEntropy(**kwargs)
...
class CrossEntropy(_WeightedLoss):
    ...
    def forward(self, predict, target, argmax=False, softmax=True):
        ...
        ), f"predict & target shape do not match. {predict.shape} != {target.shape}"
        if softmax:
...
class FocalCrossEntropy(CrossEntropy):
    def __init__(
        self, weight=None, ignore_index=None, alpha=1, gamma=2, reduction="mean"
    ):
...
class InverseFrequencyCrossEntropy(_WeightedLoss):
    ...
    def __init__(self, weight=None, ignore_index=None, reduction="mean"):
        super(InverseFrequencyCrossEntropy, self).__init__(
