...
] = "false"  # https://github.com/pytorch/pytorch/issues/57273
...
def main(args):
    ...
    with open(args.project / "config.json", "w", encoding="utf-8-sig") as f:
        arg_dict = {
            k: (str(v) if type(v) == pathlib.PosixPath else v)
    ...
    data = data.drop_duplicates(
        subset=["text_obj", "text_mthd", "text_deal", "digit"], keep="first"
    )
    ...
    # Sub Sampling
    sub_data_list, oob = bootstrap(
        train,
        args.estimators,
        dist=args.sample_dist,
        num_samples=args.num_samples,
        seed=args.seed,
    )
    logger.info(f"sub data 수: {len(sub_data_list)}")
    logger.info(
        f'sub data 카테고리 수: {[(i, len(sub_data["label"].unique())) for i, sub_data in enumerate(sub_data_list)]}'
    )
    logger.info(
        f"out of bag(oob) will be used to test. new_test = test({len(test)-len(oob)}) + oob({len(oob)}) = {len(test)}"
    )

    # Save sub datasets
    for i, sub_data in enumerate(sub_data_list):
        sub_data.to_csv(
            args.project / f"sub_data{i}.csv", index=False, encoding="utf-8-sig"
        )
    # Save test dataset
    ...
    # Column Selection
    sub_data_list = [
        sub_data[["label", "text_obj", "text_mthd", "text_deal"]]
        for sub_data in sub_data_list
    ]
    test = test[["label", "text_obj", "text_mthd", "text_deal"]]

    # Sub train-valid split
    sub_train_list, sub_valid_list = zip(
        *map(
            lambda sub_data: train_test_split(
    ...
    # Upsampling
    logger.info(
        f"sub train 수(before upsample): {[len(sub_train) for sub_train in sub_train_list]}"
    )
    if args.upsample != "":
        sub_train_list = list(
            map(
                lambda sub_train: upsample_corpus(
                    sub_train,
                    minimum=args.minimum,
                    method=args.upsample,
                    seed=args.seed,
                ),
                sub_train_list,
            )
    ...
    logger.info(
        f"sub train 수(after upsample): {[len(sub_train) for sub_train in sub_train_list]}"
    )
    ...
    for model_type, (num, i) in args.num_base_models.items():
        if num:
            backbone, tokenizer = load_backbone_tokenizer(
                model_type, max_len=args.max_len
            )
            backbones[model_type] = backbone
            tokenizers[f"{model_type}_tokenizer"] = tokenizer
            for sub_train, sub_valid in zip(
                sub_train_list[i - num : i], sub_valid_list[i - num : i]
            ):
                train_set = load_dataset(
                    model_type,
                    sub_train["text"].tolist(),
    ...
    for e in range(args.estimators):
        ...
        create_directory(
            model_args.project / "weights"
        )  # runs/train/exp00/model{e}/weights
        ...
        for epoch in range(args.epochs):
            ...
            for inputs, label in tqdm(train_loader, total=len(train_loader)):
                ...
                eval_train.update(
                    pred.tolist(), label.tolist(), loss=float(loss) * len(label)
                )
            ...
            with torch.no_grad():
                for inputs, label in tqdm(valid_loader, total=len(valid_loader)):
                    ...
                    # forward
                    output = model(**inputs)
                    target = label2target(output, label).to(
            ...
            # logging scores
            logger.info(
                f"Model {e}/{args.estimators-1} {model_args.model} | Epoch {epoch} Result"
            )
            logger.info(
                f"\ttrain | loss: {eval_train.loss}\tacc: {round(eval_train.acc, 6)}\tpc: {round(eval_train.macro_pc, 6)}\trc: {round(eval_train.macro_rc, 6)}\tf1: {round(eval_train.macro_f1, 6)}"
            )
            logger.info(
                f"\tvalid | loss: {eval_valid.loss}\tacc: {round(eval_valid.acc, 6)}\tpc: {round(eval_valid.macro_pc, 6)}\trc: {round(eval_valid.macro_rc, 6)}\tf1: {round(eval_valid.macro_f1, 6)}"
            )
            ...
            if epoch == 0:
                ...
                num_train_data, num_valid_data = [0] * len(class_id), [0] * len(
                    class_id
                )
            ...
            for metric, values in eval_train.class_scores.items():
                if metric != "class_id":
                    ...
                    for c_id, v in zip(eval_valid.class_scores["class_id"], values):
                        r = history_train[metric][
                            history_train[metric]["class_id"] == c_id
                        ][f"epoch {epoch}"].index
                        history_train[metric].loc[r, f"epoch {epoch}"] = v
                    history_train[metric].to_csv(
                        model_args.project / f"{metric}_train.csv",
                        encoding="utf-8-sig",
                        index=False,
                    )
            ...
            for metric, values in eval_valid.class_scores.items():
                if metric != "class_id":
                    ...
                    for c_id, v in zip(eval_valid.class_scores["class_id"], values):
                        r = history_valid[metric][
                            history_valid[metric]["class_id"] == c_id
                        ][f"epoch {epoch}"].index
                        history_valid[metric].loc[r, f"epoch {epoch}"] = v
                    history_valid[metric].to_csv(
                        model_args.project / f"{metric}_valid.csv",
                        encoding="utf-8-sig",
                        index=False,
                    )
            ...
            save_performance_graph(
                model_args.project / "summary.csv",
                model_args.project / "performance.png",
            )

            if best_loss is None or eval_valid.loss <= best_loss:
                logger.info(
                    f"Validation loss got better {best_loss} --> {eval_valid.loss}.  Saving model ..."
                )
                torch.save(
                    {
                        ...
                        "best acc": eval_valid.acc
                        if best_acc is None or eval_valid.acc > best_acc
                        else best_acc,
                        "best loss": eval_valid.loss
                        if best_loss is None or eval_valid.loss <= best_loss
                        else best_loss,
                        "epoch": epoch,
                    },
                ...
                pred_frame = pd.DataFrame(
                    {
                        ...
                        "category": list(
                            map(
                                lambda x: "".join(id2cat[x]), valid_loader.dataset.label
                            )
                        ),
                        "predictions": list(
                            map(lambda x: "".join(id2cat[x]), eval_valid.predictions)
                        ),
                    }
                )
                pred_frame.to_csv(
                    model_args.project / "best_loss_predictions.csv",
                    encoding="utf-8-sig",
                    index=False,
                )
                patience = 0
        ...
        for inputs, label in tqdm(test_loader, total=len(test_loader)):
            ...
            for model_type, models in base_models.items():
                for model in models:
                    ...
                    _inputs["attention_mask"] = (
                        inputs[model_type][:, 1]
                        .to(args.device, non_blocking=True)
                        .long()
                    )
                    if "bart" not in model_type:
                        _inputs["token_type_ids"] = (
                            inputs[model_type][:, 2]
                            .to(args.device, non_blocking=True)
                            .long()
                        )
            ...
            for i, base_model_output in enumerate(base_model_outputs):
                ...
                eval_base_models[i].update(
                    pred_base_model.tolist(),
                    label.tolist(),
                    loss=float(loss) * len(label),
                )
            ...
            eval_mean.update(
                pred_mean.tolist(),
                label.tolist(),
                loss=float(ensemble_loss) * len(label),
            )
    ...
    # ensemble
    logger.info(
        f"mean agg| loss: {round(eval_mean.loss, 6)}\tacc: {round(eval_mean.acc, 6)}\tpc: {round(eval_mean.macro_pc, 6)}\trc: {round(eval_mean.macro_rc, 6)}\tf1: {round(eval_mean.macro_f1, 6)}"
    )
    logger.info(
        f"vote agg| loss: -\tacc: {round(eval_vote.acc, 6)}\tpc: {round(eval_vote.macro_pc, 6)}\trc: {round(eval_vote.macro_rc, 6)}\tf1: {round(eval_vote.macro_f1, 6)}"
    )
    # base model
    for e, eval_base_model in enumerate(eval_base_models):
        logger.info(
            f"model{e} | \tacc: {round(eval_base_model.acc, 6)}\tpc: {round(eval_base_model.macro_pc, 6)}\trc: {round(eval_base_model.macro_rc, 6)}\tf1: {round(eval_base_model.macro_f1, 6)}"
        )
    ...
    with open(args.project / "test_summary.csv", "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(
            ["agg_fn", "test_loss", "accuracy", "precision", "recall", "f1score"]
        )
        wr.writerow(
            [
                "mean",
        ...
        for e, eval_base_model in enumerate(eval_base_models):
            wr.writerow(
                [
                    f"model{e}",
                    eval_base_model.loss,
                    eval_base_model.acc,
                    eval_base_model.macro_pc,
                    eval_base_model.macro_rc,
                    eval_base_model.macro_f1,
                ]
            )
    ...
    for metric, values in eval_mean.class_scores.items():
        if metric != "class_id":
            ...
            history_frame.to_csv(
                args.project / f"test_result_mean.csv",
                encoding="utf-8-sig",
                index=False,
            )
    ...
    for metric, values in eval_vote.class_scores.items():
        if metric != "class_id":
            ...
            for c_id, v in zip(eval_vote.class_scores["class_id"], values):
                ...
                history_frame.loc[r, metric] = v
            history_frame.to_csv(
                args.project / f"test_result_vote.csv",
                encoding="utf-8-sig",
                index=False,
            )
    ...
    pred_frame = pd.DataFrame(
        {
            ...
            "category": list(
                map(lambda x: "".join(id2cat[x]), test_loader.dataset.label)
            ),
    ...
    pred_frame.to_csv(
        args.project / "predictions.csv", encoding="utf-8-sig", index=False
    )
...
def get_args():
    ...
    parser.add_argument(
        "--root",
        default=DATA / "1. 실습용자료_final.txt",
        type=str,
    ...
    # Data Preprocess
    parser.add_argument(
        "--num-test", default=50000, type=int, help="the number of test data"
    )
    parser.add_argument(
        "--upsample", default="", type=str, help='"shuffle", "uniform", "random"'
    )
    parser.add_argument(
        "--minimum",
        default=100,
    ...
    parser.add_argument(
        "--n-kobert", default=0, type=int, help="a number of kobert base models"
    )
    parser.add_argument(
        "--n-bert", default=0, type=int, help="a number of kobert base models"
    )
    parser.add_argument(
        "--n-mlbert",
        default=0,
    ...
    parser.add_argument(
        "--n-kogpt2", default=0, type=int, help="a number of kogpt2 base models"
    )
    parser.add_argument(
        "--n-kogpt3", default=0, type=int, help="a number of kogpt2 base models"
    )
    parser.add_argument(
        "--n-electra", default=0, type=int, help="a number of kogpt2 base models"
    )
    parser.add_argument(
        "--n-funnel", default=0, type=int, help="a number of kogpt2 base models"
    )

    # n layers
    parser.add_argument("--dr-rate", default=None, type=float, help="")
    parser.add_argument(
        "--n-layers",
        default=1,
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        help='Available schedulers are "linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup',
    )
    parser.add_argument("--warmup-ratio", default=0.02, type=int, help="lr-scheduler")

    # Optimizer
    parser.add_argument(
        "--optimizer", type=str, default="AdamW", help="default is AdamW"
    )
    ...
    parser.add_argument(
        "--seed", default=5986, type=int, help="seed for initializing training."
    )
    parser.add_argument(
        "--max-len", default=50, type=int, help="max sequence length to cut or pad"
    )
