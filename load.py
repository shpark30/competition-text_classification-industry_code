...
def load_backbone_tokenizer(model_type, max_len=50):
    ...
    if model_type == "mlbert":
        mlbert = BertForSequenceClassification.from_pretrained(
            "bert-base-multilingual-cased"
        )
        mlbert = mlbert.bert
        mlbert_tokenizer = BertTokenizer.from_pretrained(
    ...
    if model_type == "asbart":
        asbart = AsianBartForConditionalGeneration.from_pretrained(
            "hyunwoongko/asian-bart-ecjk"
        )
        asbart = asbart.model
        asbart_tokenizer = AsianBartTokenizer.from_pretrained(
            "hyunwoongko/asian-bart-ecjk"
        )
    ...
    if model_type == "kogpt3":
        kogpt3_tokenizer = BertTokenizerFast.from_pretrained(
...
def load_dataset(model_type, text, label, tokenizer, max_len=50):
    if model_type == "kobert":
        return KobertClassifyDataset(text, label, tokenizer)
...
def load_backbones_tokenizers_classifiers(exp_paths, num_classes=225, device="cpu"):
    ...
    for path in exp_paths:
        try:
            checkpoints.append(
                torch.load(os.path.join(path, "weights/best_loss.pth.tar"))
            )
        except:
    ...
    for i, model_type in enumerate(list(set(base_model_types))):
        backbone, tokenizer = load_backbone_tokenizer(
            model_type, max_len=args_list[i]["max_len"]
        )
